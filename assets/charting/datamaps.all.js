(function() {
  ...
  var defaultOptions = {
    ...
    geographyConfig: {
        ...
        popupTemplate: function (geography, data) {
            var name = (geography.properties != null && geography.properties.name != null) ? geography.properties.name : geography.id;
            if (data == null) {
                return '<div class="hoverinfo"><strong>' + name + '</strong><br/><span>No Data</span></div>';
            }
            return '<div class="hoverinfo"><strong>' + name + '</strong><br/><span>' + _.str.numberFormat(data.dataValue) + '</span></div>';
        },
        popupOnHover: true,
  ...
  function setProjection( element, options ) {
    ...
    if ( options.scope === 'usa' ) {
      ...
      projection = d3.geo[options.projection]()
        .scale((width + 1) / 2.2 / Math.PI)
        .translate([width / 2, height / (options.projection === "mercator" ? 1.45 : 1.8)]);
    } else if (options.scope === 'india') {
        var toUse = (width < height) ? width : height;
        projection = d3.geo.mercator().
            scale(toUse * 1.5)
            .translate([toUse * -1.65, toUse]);
    }
  ...
  function drawSubunits( data ) {
    ...
    var geoData = (this.options.scope == "world" || this.options.scope == "usa" || this.options.scope == "nga" || this.options.scope == "Eth_Region_2013") ? topojson.feature( data, data.objects[ this.options.scope ] ).features : data.features;
    ...
    var geo = subunits.selectAll('path.datamaps-subunit').data( geoData );
      ...
      geo.enter()
          ...
          .attr('class', function(d) {
              return 'datamaps-subunit ' + options.identifierPath(d);
          })
          .attr('data-info', function(d) {
              return JSON.stringify(colorCodeData[options.identifierPath(d)]);
          })
          .style('fill', function(d) {
              var fillColor;

              if (colorCodeData[options.identifierPath(d)]) {
                  fillColor = fillData[colorCodeData[options.identifierPath(d)].fillKey];
              }
          ...
          //    var opacity = 1;
          //    if (colorCodeData[d.id] != null){
          //        opacity = colorCodeData[d.id].opacity;
          //    }
          //    return opacity;
          //})
          .style('stroke-width', geoConfig.borderWidth)
          .style('stroke', geoConfig.borderColor)
          .on("click", this.options.clickHandler);
  }
